...
ensure_host_device_override "$PI_VIDEO_DEVICES_ENABLED_VAL" "$CEC_ENABLED_VAL"
PI_VIDEO_NODES_MISSING=""
if [ "$PI_VIDEO_DEVICES_ENABLED_VAL" = "1" ]; then
  PI_VIDEO_NODES_MISSING="$(check_pi_video_nodes)"
fi
CEC_NODES_DETECTED="$(detect_cec_device_nodes | paste -sd, - 2>/dev/null || true)"
CEC_NODES_MISSING="0"
...
if [ "$CEC_NODES_MISSING" = "1" ]; then
  say "  WARN: RELAYTV_CEC_ENABLED=1 but no /dev/cec* device was found."
fi
